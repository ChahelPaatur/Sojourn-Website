...
export default function TeamMember({ name, role, bio, image, social, index }: TeamMemberProps) {
  ...
  return (
    ...
    >
      <motion.div
        className="relative rounded-2xl overflow-hidden backdrop-blur-lg bg-black/20 p-6 border border-white/10"
        animate={isClicked ? {
          boxShadow: [
      ...
      >
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-br from-[#FFD700]/20 to-transparent animate-pulse" />
          <img
            src={image}
            alt={name}
        ...
        <div className="flex justify-center space-x-4">
          {social.map((link) => (
            ...
            >
              <motion.div
                className="absolute -inset-2 bg-[#FFD700]/20 rounded-full blur-sm"
                initial={{ opacity: 0 }}
